...
SlaveRequest::SlaveRequest( SlaveFile *file, const QByteArray &request, const QVariant &parameters, const Callback &callback )
	...
	if ( callback.getFailureSlot() )
		connect( this, SIGNAL( requestFailure( QString, int ) ), callback.getTarget(), callback.getFailureSlot() );
	if ( callback.getSuccessSlot() )
		connect( this, SIGNAL( requestSuccess( QVariantMap ) ), callback.getTarget(), callback.getSuccessSlot() );
}
