...
public:
	...
	inline void setContent(const QByteArray& content) { mContent = content; }
	inline void addContent(const char* data, int length) { mContent.append(data, length); }
	inline const QByteArray& getContent() const { return mContent; }

	inline void setRevision(int revision) { mRevision = revision; }
	inline int getRevision() const { return mRevision; }

	inline void setUndoLength(int undoLength) { mUndoLength = undoLength; }
	...
	inline void triggerProgress(int progress) { mCallback.triggerProgress(progress); }
