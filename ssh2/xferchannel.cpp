...
XferChannel::XferChannel( SshHost *host, bool sudo )
    : SlaveChannel( host, sudo ), mInternalStatus( _WaitingForRequests ) {
...
ShellChannel::ReadReply XferChannel::readBinaryData( int size ) {
	...
	while ( mBinaryReadBuffer.length() < size ) {
		...
		int transferred = Tools::unbin( mBinaryReadBuffer, mReadBuffer, size, mReadBuffer.length(), &mLeftoverEscape );
		...
		else if ( rc == LIBSSH2_ERROR_EAGAIN || ( rc == -1 && libssh2_session_last_errno( mSession->sessionHandle() ) == LIBSSH2_ERROR_EAGAIN ) ) {
