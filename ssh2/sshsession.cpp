...
SshSession::SshSession( SshHost *host )
    : mThreadEndedCalled( false ), mSocket( 0 ), mHandle( 0 ), mSocketReadNotifier( 0 ), mSocketExceptionNotifier( 0 ), mAtChannelLimit( false ) {
...
bool SshSession::authenticateAgent() {
	...
	try {
		...
		while ( 1 ) {
			...
			if ( rc )
				throw( rc == 1 ? QObject::tr( "No identities stored in SSH agent were accepted!" ) : QObject::tr( "Failed to receive identity from SSH agent!" ) );
