...
bool SshSession::openSocket() {
	...
	if ( mHost->getCachedIpAddress() ) {
		SSHLOG_TRACE( mHost ) << "Atempting to connect using a cached IP address:" << Tools::stringifyIpAddress(
			mHost->getCachedIpAddress() );
...
bool SshSession::authenticatePassword( bool keyboardInteractive ) {
	...
	while ( true ) {
		...
		options.insert( "blurb",
		                QObject::tr( passwordRejected ? "Your password for %1 was rejected. Please try again." :
		                             "Please enter your password for %1 below." ).arg( mHost->getName() ) );
...
bool SshSession::authenticateAgent() {
	...
	try {
		...
		if ( ! agent ) {
			throw( QObject::tr( "Failed to call libssh2_agent_init!" ) );
		}
...
bool SshSession::authenticate() {
	...
	if ( cachedMethod ) {
		...
		if ( ! mHost->getPassword().isEmpty() ||
		     ( cachedMethod != AuthPassword && cachedMethod != AuthKeyboardInteractive ) ) {
			SSHLOG_TRACE( mHost ) << "Attempting to use last successful authentication method: " <<
			        mHost->getCachedAuthMethod();
