...
// Guess at the max number of channels per connection
#define CHANNEL_LIMIT_GUESS 10

// When slave-channel:slave-queue-length ratio is off by at least this multiplier, think about opening additional slave
// channels
#define SLAVE_CHANNEL_QUEUE_MULTIPLIER 3

// Maximum number of slave channels per host
#define MAX_SLAVE_CHANNELS 3
...
class SshHost : public QObject {
	...
	public:
		...
		inline Status getOverallStatus() const {
			return mOverallStatus;
		}

		inline QString getConnectionString() const {
			return mConnectionString;
		}

		void sendSlaveRequest( bool sudo,
		                       SlaveFile* file,
		...
		LogHelper log( QsLogging::Level level ) {
		...
		inline void lockNewSessions() {
			mNewSessionMutex.lock();
		}

		inline void unlockNewSessions() {
			mNewSessionMutex.unlock();
		}

		bool waitBeforeCheckingSlave( SshChannel* channel );    // Returns true if the specified channel should
		                                                        // wait
		                                                        // before checking slave (first check only)
		...
		inline unsigned long getCachedIpAddress() const {
			return mCachedIpAddress;
		}

		inline SshSession::AuthMethod getCachedAuthMethod() const {
			return mCachedAuthMethod;
		}

		inline int getChannelLimitGuess() const {
		...
		inline void setCachedAuthMethod( SshSession::AuthMethod authMethod ) {
			mCachedAuthMethod = authMethod;
		}

		inline void setChannelLimitGuess( int guess ) {
		...
		inline bool getSaveHost() const {
			return mSaveHost;
		}

		inline bool getSavePassword() const {
			return mSavePassword;
		}

		inline bool getSaveKeyPassphrase() const {
			return mSaveKeyPassphrase;
		}
	...
	protected:
		...
		void failSlaveRequests( const QString& error,
		                        int flags,
		                        QMutex& listLock,
	...
	private:
		...
		Status mOverallStatus;  // This is updated as sessions connect and disconnect to reflect the current
		                        // overall status of this ssh host.
		QString mConnectionString;      // During connection, this string is updated to reflect the current
		                                // phase of connection.
		bool mOverallStatusDirty;       // Used to allow SshHost overall status to lazy-update. So, multiple
		                                // rapid status changes will only trigger 1 update.
		QMutex mNewSessionMutex;        // In the interests of UI sanity, only allow 1 new SSH session to
