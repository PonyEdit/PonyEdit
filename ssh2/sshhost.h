...
#define SLAVE_CHANNEL_QUEUE_MULTIPLIER 3	/* When slave-channel:slave-queue-length ratio is off by at least this
						 * multiplier, think about opening additional slave channels */
...
bool waitBeforeCheckingSlave( SshChannel* channel );		// Returns true if the specified channel should wait
...
Status mOverallStatus;		// This is updated as sessions connect and disconnect to reflect the current overall
				// status of this ssh host.
QString mConnectionString;		// During connection, this string is updated to reflect the current phase of
					// connection.
bool mOverallStatusDirty;		// Used to allow SshHost overall status to lazy-update. So, multiple rapid
					// status changes will only trigger 1 update.
