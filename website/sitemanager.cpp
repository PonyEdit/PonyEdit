...
void SiteManager::handleUpdateCheckReply( QList< QVariant > reply, bool forceNotification ) {
	...
	foreach ( QVariant rawEntry, reply ) {
		...
		if ( major > MAJOR_VERSION || ( major == MAJOR_VERSION && minor > MINOR_VERSION ) || ( major == MAJOR_VERSION && minor == MINOR_VERSION && revision > REVISION ) ) {
			...
			if ( major > update_major || ( major == update_major && minor > update_minor ) || ( major == update_major && minor == update_minor && revision > update_revision ) ) {
