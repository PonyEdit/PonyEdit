...
SshServerOptionsWidget::SshServerOptionsWidget( QWidget *parent )
    : OptionsDialogPage( parent ), ui( new Ui::SshServerOptionsWidget ) {
	ui->setupUi( this );
	...
	connect( ui->serversList, SIGNAL( currentItemChanged( QListWidgetItem *, QListWidgetItem * ) ), this, SLOT( serverClicked( QListWidgetItem *, QListWidgetItem * ) ) );
...
QListWidgetItem *SshServerOptionsWidget::populateServers() {
	...
	for ( int i = 0; i < ui->serversList->count(); i++ ) {
		...
		if ( !currentList.value( host, true ) ) {
			...
			connect( mConfigWidgets[ i ], SIGNAL( nameUpdated( QString ) ), this, SLOT( serverNameUpdated( QString ) ) );
