...
bool SyntaxDefXmlHandler::startElement( const QString & /* namespaceURI */,
	...
	case Language | General:
		if ( localName.compare( "comments", Qt::CaseInsensitive ) == 0 ) {
			...
			mDefinition->setAdditionalDeliminators( Tools::getStringXmlAttribute( atts,
			                                                                      "additionaldeliminators" ) );
			mDefinition->setWordWrapDeliminator( Tools::getStringXmlAttribute( atts,
			                                                                   "wordwrapdeliminator" ) );
		}
		break;
	...
	case Language | General | Comments:
		if ( localName.compare( "comment", Qt::CaseInsensitive ) == 0 ) {
			 ...
			 *  style->multiline = (Tools::getStringXmlAttribute(atts, "name").compare("multiline",
