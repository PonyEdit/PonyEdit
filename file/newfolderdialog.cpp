...
NewFolderDialog::NewFolderDialog( QWidget *parent, const Location &parentLocation )
    : QDialog( parent ), ui( new Ui::NewFolderDialog ) {
...
void NewFolderDialog::attempt( bool sudo ) {
	...
	loc.createNewDirectory( ui->folderName->text(), Callback( this, SLOT( createSuccess( QVariantMap ) ), SLOT( createFailure( QString, int ) ) ) );
}
