...
void SlaveFile::finalizeFileOpen() {
	...
	if ( mSlaveOpenResults.isEmpty() || mDownloadedChecksum.isNull() )
		return;
...
void SlaveFile::slaveReconnectFailure( QString error, int flags ) {
	...
	if ( flags & SlaveRequest::ConnectionError )
		...
		gDispatcher->emitGeneralErrorMessage( mLocation.getLabel() + " has encountered a serious error, and cannot be recovered. " + "Please save the file under a different name or on a different host, close it and try reconnecting. Error: " + error );
}
