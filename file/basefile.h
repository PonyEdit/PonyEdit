...
int mLastSavedUndoLength;		// Undo length is used only for detecting unsaved changes; it helps work out if
					// the user has undone all unsaved changes
...
static QMutex sDeletionLock;		// Used to prevent cross-thread operations sensitive to the sudden deletion of
					// files from getting upset.
};
