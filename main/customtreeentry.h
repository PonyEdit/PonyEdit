...
public:
	...
	inline int getIndexWithinParent() const { return mIndex; }
	...
	inline void setHover(bool hover) { mHover = hover; }
	inline bool isHover() { return mHover; }
	...
	template <class T> void setAutoDeleteData(T data){ mDataDeleteProc = (deleteData<T>); mData = static_cast<void*>(data); }
	template <class T> void setData(T data) { mDataDeleteProc = NULL; mData = static_cast<void*>(data); }
	template <class T> T getData() const { return static_cast<T>(mData); }

	//	Expansion / collapsing stuff
	void setExpanded(bool expanded);
	void setExpandable(bool expandable);
	inline bool isExpandable() const { return mExpandable || mChildren.length(); }
	void setDelayedLoad(QObject* callbackTarget, const char* loadSlot);

	//	What I look like
	inline const QIcon& getIcon() const { return mStaticIcon; }
	inline const QString& getLabel() const { return mStaticLabel; }
	virtual bool isCustomDrawn() const { return false; }
