...
RegExpTester::RegExpTester( QWidget *parent )
    : QWidget( parent ), ui( new Ui::RegExpTester ) {
	ui->setupUi( this );
	...
	ui->regexpType->addItem( tr( "W3C XML Schema 1.1" ), QVariant( static_cast< int >( QRegExp::W3CXmlSchema11 ) ) );
...
void RegExpTester::updateResult() {
	...
	QRegExp regExp = QRegExp( ui->regexp->text(), ui->caseSensitive->isChecked() ? Qt::CaseSensitive : Qt::CaseInsensitive,
	    static_cast< QRegExp::PatternSyntax >( ui->regexpType->itemData( ui->regexpType->currentIndex() ).toInt() ) );
